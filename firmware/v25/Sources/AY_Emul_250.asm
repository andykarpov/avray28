; AY-3-8912 IC emulator version 25.0 for ATMega8/48/88/168/328 10.09.2019
;
...
ENVELOPE_GENERATOR_END:
	...
	dec		CntN
	lsr		NoiseAddon		; shift right -> load to carry
	mov		NoiseAddon,RNGL
	ror		RNGH			; rotate through carry
	ror		RNGL			; rotate through carry
...
CH_C_NO_CHANGE:
	...
	/// MIXER
	/////////////////////////////////////////////////////////////////////////////////////
	lds		TMP2,AY_REG07	; Load Mixer AY Register from SRAM
	com		TMP2			; invert mixer register bits
	and		TMP2,TNLevel 	; combine inverted mixer and levels
	
	lds		TMP,AY_REG07	; Load Mixer AY Register from SRAM
    mov		YL,TMP  		; and noise and tone bits in mixer
	lsl		YL
	swap	YL
	and		TMP,YL
	/////////////////////////////////////////////////////////////////////////////////////
	...
	ldd		OutA,Y+0x20		; load volume value from SRAM 0x220 + YL (0x230+amplitude)
	sbrc	TMP,b0			; if both tone and noise is disabled in mixer volume should be unchanged (controlled by amplitude)
	rjmp	CH_A_END	
	mov		YL,TMP2			; check tone and noise level (TMP2)
	andi	YL,9
	brne	CH_A_NOT0
	clr		OutA			; set volume to 0 if both are silent
CH_A_NOT0:
	cpi		YL,9
	brne	CH_A_END
	lsl		OutA			; increase volume by 2 if both tone and noise enabled
CH_A_END:
	...
	sbrc	TMP,b1			; if both tone and noise is disabled in mixer volume should be unchanged (controlled by amplitude)
	rjmp	CH_B_END	
	mov		YL,TMP2			; check tone and noise level (TMP2)
	andi	YL,18
	brne	CH_B_NOT0
	clr		OutB			; set volume to 0 if both are silent
CH_B_NOT0:
	cpi		YL,18
	brne	CH_B_END
	lsl		OutB			; increase volume by 2 if both tone and noise enabled
CH_B_END:
	...
	sbrc	TMP,b2			; if both tone and noise is disabled in mixer volume should be unchanged (controlled by amplitude)
	rjmp	CH_C_END	
	mov		YL,TMP2			; check tone and noise level (TMP2)
	andi	YL,36
	brne	CH_C_NOT0
	clr		OutC			; set volume to 0 if both are silent
CH_C_NOT0:
	cpi		YL,36
	brne	CH_C_END
	lsl		OutC			; increase volume by 2 if both tone and noise enabled
CH_C_END:
